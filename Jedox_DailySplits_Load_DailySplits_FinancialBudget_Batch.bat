...
if exist DailySplitsEimportDailySplitsFinancialBudgetBatch*.log (
  del DailySplitsEimportDailySplitsFinancialBudgetBatch*.log
)
...
set LogFileName="%ret%\DailySplitsEimportDailySplitsFinancialBudgetBatch-%DateN%_%timen%.log"
...
::
:: Capture the current/budget year
...
IF %DebugFlag% EQU 0 (
  call .\etlclient -p "[04-04]-PAS-FINANCIALS [Daily Splits by Dispatch Type]" -j "Load Financial Budget Daily Splits" >>%LogFileName%
)
GOTO SendEmail
...
set OutFile=DailySplitsBudgetLoadSendMail.ps1
...
IF %EmailReturnCode% EQU 0 (
  set SubjectText="Step 3 Jedox Daily Spits Financial Budget Import  Batch - Daily Splits Import Successful"
)
IF %EmailReturnCode% EQU 1 (
  set SubjectText="Step 3 Jedox Daily Spits Financial Budget Import  Batch - The Daily Splits Import completed with warnings"
)
IF %EmailReturnCode% EQU 2 (
  set SubjectText="Step 3 Jedox Daily Spits Financial Budget Import  Batch - The Daily Splits Import failed! - Investigate NOW!"
)
IF %EmailReturnCode% EQU 999 (
  set SubjectText="Step 3 Jedox Daily Spits Financial Budget Import  Batch - The Update of Paramters Failed! - Investigate NOW! "
)
...
:: Send the email using the file DailySplitsBudgetLoadSendMail.ps1 which has just been "written" above
Start /wait Powershell.exe -executionpolicy remotesigned -File "C:\Program Files\Jedox\Jedox Suite\tomcat\client\DailySplitsBudgetLoadSendMail.ps1"
