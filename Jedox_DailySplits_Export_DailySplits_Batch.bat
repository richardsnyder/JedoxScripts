...
if exist DailySplitsExportDailySplitsBatch*.log (
  del DailySplitsExportDailySplitsBatch*.log
)
...
set LogFileName="%ret%\DailySplitsExportDailySplitsBatch-%DateN%_%timen%.log"
...
IF %DebugFlag% EQU 0 (
  call .\etlclient -p "[04-04]-PAS-FINANCIALS [Daily Splits by Dispatch Type]" -j "Export Daily Splits and Profit Centres" -c ForecastFinancialYear=%CurrYear% BudgetFinancialYear=%BudYear% >>%LogFileName%
)
GOTO SendEmail
...
set OutFile=DailySplitsExportSendMail.ps1
...
IF %EmailReturnCode% EQU 0 (
  set SubjectText="Step 1 Jedox Daily Spits Export  Batch - Daily Splits Export Successful"
)
IF %EmailReturnCode% EQU 1 (
  set SubjectText="Step 1 Jedox Daily Spits Export  Batch - The export completed with warnings"
)
IF %EmailReturnCode% EQU 2 (
  set SubjectText="Step 1 Jedox Daily Spits Export  Batch - The export failed! - Investigate NOW!"
)
IF %EmailReturnCode% EQU 999 (
  set SubjectText="Step 1 Jedox Daily Spits Export  Batch - The Update of Paramters Failed! - Investigate NOW! "
)
...
set BodyText="Daily splits have been exported to the data warehouse. Log files are attached."
...
:: Send the email using the file DailySplitsExportSendMail.ps1 which has just been "written" above
Start /wait Powershell.exe -executionpolicy remotesigned -File "C:\Program Files\Jedox\Jedox Suite\tomcat\client\DailySplitsExportSendMail.ps1"
